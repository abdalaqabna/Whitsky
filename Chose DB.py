...
class ConfigApp(tk.Tk):
    def __init__(self):
        ...
        self.db_option_menu = tk.OptionMenu(self, self.db_type, *db_options, command=self.update_ui)
        self.db_option_menu.grid(row=0, column=1, pady=10, padx=10)
        ...
        self.host_label.grid(row=3, column=0, pady=10, padx=10)
        self.host_entry = tk.Entry(self)
        self.host_entry.grid(row=3, column=1, pady=10, padx=10)

        self.port_label = tk.Label(self, text="Port:")
        self.port_label.grid(row=4, column=0, pady=10, padx=10)
        ...
        self.update_ui(self.db_type.get())  # Initial UI update

    def update_ui(self, db_type):
        if db_type == "oracle":
    ...
    def connect(self):
        ...
        host = self.host_entry.get() if self.host_entry.winfo_ismapped() else ""
        port = self.port_entry.get() if self.port_entry.winfo_ismapped() else ""
